...
namespace EastElite.DCS.Ajax
{
    public class ServiceBase : IHttpHandler
    {
        ...
        public bllOperateLogInfo operatelog = new bllOperateLogInfo();
        private string token = "EASTELITE";
        ...
        protected string GetQueryUrl()
        {
            ...
            for (int i = 0; i < Pagcontext.Request.Params.Count; i++)
            {
                if (Pagcontext.Request.Params.Keys[i] != null &&(Pagcontext.Request.Params.Keys[i].ToString().ToUpper() == "ALL_HTTP"||Pagcontext.Request.Params.Keys[i].ToString().ToUpper()=="LOGINUSER"))
                {
                    break;
                }
                else if (Pagcontext.Request.Params.Keys[i] != null && Pagcontext.Request.Params.Keys[i].ToString() != "actionname")
                {
                    strparameters += "&" + Pagcontext.Request.Params.Keys[i].ToString();
                    strparameters += "=" + Pagcontext.Request.Params[i].ToString();
                }
            }
        ...
        protected string CheckOptionalParameters(Dictionary<string, EnumSearchType> parameters, string filter = " WHERE 1=1 ")
        {
            ...
            if (parameters != null && parameters.Count > 0)
            {
                ...
                {
                    if (Pagcontext.Request.Form[param.Key] != null && Pagcontext.Request.Form[param.Key].ToString() != "")
                    {
                        switch (param.Value)
                        {
                            ...
                            case EnumSearchType.AndSTime:
                                filter += " AND " + "CONVERT(varchar(100), HandledDate, 23)" + " >= '" + Pagcontext.Request.Form[param.Key].ToString() + "'";
                                break;
                            case EnumSearchType.AndETime:
                                filter += " AND " + "CONVERT(varchar(100), HandledDate, 23)" + " <= '" + Pagcontext.Request.Form[param.Key].ToString() + "'";
                                break;
                            case EnumSearchType.AndInInt:
                                filter += " AND " + param.Key +" IN(" + Pagcontext.Request.Form[param.Key].ToString() + ")";
                                break;
                            case EnumSearchType.AndInString:
                                filter += " AND " + param.Key + " IN(" + Pagcontext.Request.Form[param.Key].ToString() + ")";
                                break;
                        }
                    }
        ...
        protected bool CheckActionParameters(Dictionary<string, object> dicPar, List<string> liPra)
        {
            ...
            if (Flag)
            {
                ...
                {
                    logentity.HandledID = dicPar["U_ID"].ToString();
                }
                //获取操作人Url
